...
- **Pixel office** -- Canvas-rendered workstations that show each agent's state at a glance (working, waiting, needs attention, idle, disconnected). Desks are grouped into per-repo pods, each on its own labeled rug, with desks paired up facing each other across a shared aisle like a real office; leftover floor space fills with ambient sofa-and-plant decor that yields as the office gets crowded. Pixel-art characters (six sprite variants) face the screen when working, turn around in their chairs when waiting, and stand up when idle. The three whiteboards mirror the job board live: one pinned paper per job in each column, with a "+N" note when more are queued than fit.
