...
// PR, so a missing field reads as true. A scheduled card never requires one.
export function jobRequiresPr(job) {
  return !isScheduled(job) && job?.requiresPr !== false;
}
...
export function createJob({ title, detail, repoPath, type, schedule, permissionMode, agent, requiresPr, postedBy, postedByName, postedByAgent }) {
  ...
  return {
    job: {
      ...
      // Never false on a scheduled card, so one later turned one-time is not
      // silently a no-PR card nobody chose.
      requiresPr: resolved.type === 'scheduled' || requiresPr !== false,
...
export function buildJobPrompt(job) {
  ...
  parts.push('', '---', ...(isScheduled(job) ? scheduledPromptSuffix(job) : oneTimePromptSuffix(jobAgent(job), jobRequiresPr(job))));
  return parts.join('\n');
}
...
function oneTimePromptSuffix(agent = DEFAULT_JOB_AGENT, requiresPr = true) {
...
//
// A one-time job ends when its agent calls finish_job (or its PR appears) — an
// explicit signal. A scheduled run has no such artefact: it may write no code, so
