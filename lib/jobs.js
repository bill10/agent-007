...
const PERMISSION_FLAG_ALIASES = { codex: { '-s': '--sandbox', '-a': '--ask-for-approval' }, claude: {} };

// The permission flags among `tokens` for `agent`, normalised to their long
// form, in order of appearance, or [] when the agent is unknown. Anything not
// in the allowlist — an unknown flag, a value the CLI would reject, a bare
// switch given a value — is dropped, so what comes back can be put on a
// command line verbatim. Used both on a fresh command (to record what the
// agent ran with) and on a stored record (config.json is hand-editable), so
// the two can never disagree about what is a permission.
export function normalizePermissionFlags(agent, tokens) {
  ...
  for (let i = 0; i < tokens.length; i++) {
    ...
    let [flag, inline] = raw.includes('=') ? [raw.slice(0, raw.indexOf('=')), raw.slice(raw.indexOf('=') + 1)] : [raw, undefined];
    flag = aliases[flag] || flag;
    if (!Object.prototype.hasOwnProperty.call(table, flag)) continue;
    const values = table[flag];
    if (values === null) { if (inline === undefined) out.push(flag); continue; }
    const value = inline !== undefined ? inline : tokens[++i];
    if (values.includes(value)) out.push(flag, value);
  }
  return out;
}
...
// dies on the spot. `codex resume --last` is scoped to the cwd by default, so
// it lands on that worktree's own session rather than the newest one anywhere.
//
...
export function resumeCommand(agent, mode, flags) {
  ...
  if (agent === 'codex') {
    ...
    return `codex resume --last${flag ? ` ${flag}` : ''}`;
  }
...
export function createJob({ title, detail, repoPath, type, schedule, permissionMode, agent, postedBy, postedByName, postedByAgent }) {
...
export function buildJobPrompt(job) {
  ...
  parts.push('', '---', ...(isScheduled(job) ? scheduledPromptSuffix(job) : oneTimePromptSuffix(jobAgent(job))));
  return parts.join('\n');
}
...
function oneTimePromptSuffix(agent = DEFAULT_JOB_AGENT) {
...
export function buildJobCommand(job, { permissionMode = DEFAULT_PERMISSION_MODE } = {}) {
  ...
  if (jobAgent(job) === 'codex') {
    ...
    const flag = CODEX_MODE_FLAGS[mode];
    return `codex ${flag ? `${flag} ` : ''}${quote(buildJobPrompt(job))}`;
  }
