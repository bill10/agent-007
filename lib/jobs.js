...
// Board agents run with permissions bypassed. The reason is that `auto`, the
// earlier default, silently becomes Manual where it is unavailable — and
// Manual is the one mode a dispatched agent cannot work in.
//
// Auto mode needs a supported model and an organisation that has not turned it
// off. Where either fails (an Amazon Bedrock or Vertex account on an
// unsupported model, `permissions.disableAutoMode` in managed settings, or
// Anthropic disabling it server-side), Claude Code does NOT fail and does not
// exit: "When the flag, a settings file, or the built-in default selects auto
// but auto mode isn't available to the session, Claude Code starts the session
// in Manual instead" (docs/en/permission-modes). So the agent spawns fine,
// starts working, and stops at its first permission prompt — with nobody
// there. The card reaches In progress and then sits on `needs-input`, or
// `stalled` once the quiet window passes, for every job, forever. Observed on
// a Bedrock-only machine.
//
// Note what this failure is NOT, because the first version of this comment got
// it wrong: the agent does not die, the card is not stranded with a null
// `lastError`, and the board does surface it. It is a stall, not a crash, and
// deriveJobStatus already shows it.
//
// Do not read the fix as "the board is unattended now" either. Claude Code
// still shows the workspace-trust dialog the first time it runs in any
// directory, and every board agent gets a brand-new worktree, so it fires on
// EVERY job; bypassPermissions does not skip it (verified against 2.1.250 —
// see the trust pre-seed entry in TODOS.md).
//
// The cost is real and is recorded in DESIGN.md, not hidden here: an agent in
// this mode runs Bash unprompted as the user, so nothing in the app bounds a
// hostile one and the classifier that used to review each action is gone. The
// board's card-ownership check is a guard against misbehaviour, not a security
// boundary. The right long-term shape is a per-job mode with the board setting
// as its default, back to `auto` — see the permission-mode entry in TODOS.md.
//
// Unverified on a Bedrock machine, and worth checking before relying on it:
// whether bypassPermissions is itself reachable there. The docs gate it behind
// a one-time responsibility dialog (saved to user settings), managed settings
// can disable it, and Claude Code refuses it as root or under sudo — and at
// least in the VS Code extension an ungranted bypass ALSO starts the session
// in Manual rather than erroring.
...
export function createJob({ title, detail, repoPath, type, schedule, postedBy, postedByName, postedByAgent }) {
