...
export const DISPATCH_INTERVAL_MS = 3 * 60 * 1000;  // scan cadence
export const MAX_AGENTS_PER_REPO = 2;               // concurrent board agents
export const STALLED_AFTER_MS = 3 * 60 * 1000;      // quiet WAITING -> "stalled"

// Board agents run with acceptEdits: they are confined to their own worktree on
// a throwaway branch and everything lands in a reviewable PR, so auto-accepting
// file edits removes the most common stall without granting shell access.
...
export function createJob({ title, detail, repoPath, postedBy, postedByName }) {
...
// How many board agents a repo currently has in flight. Two exclusions, both
// about not deadlocking the queue:
//  - review jobs: the PR is open, so the agent (left alive for revisions) is no
//    longer holding a slot.
//  - in-progress jobs whose agent has died: a session that no longer exists
//    cannot be occupying a slot. Without this, one crashed agent would block its
//    repo forever, since we deliberately never auto-revert a card to To do.
...
// Pick which to-do jobs to dispatch this tick, oldest first, respecting the
// per-repo cap. Jobs over the cap simply stay in To do and are reconsidered
// next tick — nothing is dropped or reordered permanently.
...
export function buildJobPrompt(job) {
  ...
  parts.push(
    ...
    '',
    'When the work is complete, commit it and open a pull request (`gh pr create`).',
    'The board watches for that PR and moves this job to Review when it appears.',
  );
  return parts.join('\n');
}
...
export function buildJobCommand(job, { permissionMode = DEFAULT_PERMISSION_MODE } = {}) {
  ...
  return `claude --permission-mode ${permissionMode} "${escaped}"`;
}
