...
// is. Codex has no --permission-mode: the board's mode maps onto its one
// flag (bypassPermissions -> --dangerously-bypass-approvals-and-sandbox) and
// everything else runs it with no flag at all, its own default approvals.
...
// plain shell) is claude, the board's own default.
export function jobAgentFromCommand(command) {
  return /^codex\b/.test(String(command || '').trim()) ? 'codex' : DEFAULT_JOB_AGENT;
}
...
export function resolveJobAgent(agent) {
  ...
  if (!isValidJobAgent(agent)) {
    return { error: `Unknown agent "${agent}" \u2014 expected one of: ${JOB_AGENTS.join(', ')}` };
  }
  return { agent };
}
...
export function buildJobCommand(job, { permissionMode = DEFAULT_PERMISSION_MODE } = {}) {
  ...
  if (jobAgent(job) === 'codex') {
    // No --add-dir: Codex's sandbox reads anywhere on disk, it only gates
    // writes, and attachments are only ever read.
    const flag = mode === 'bypassPermissions' ? '--dangerously-bypass-approvals-and-sandbox ' : '';
    return `codex ${flag}${quote(buildJobPrompt(job))}`;
  }
