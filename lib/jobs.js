...
// A scheduled job is not necessarily code. "Run /review then /ship" is wrong
// for a card that summarises yesterday's commits or checks a dependency feed:
// it would push the agent into inventing a change so it had something to open a
// pull request with. So the whole review/ship instruction is gone, and what
// replaces it is about finishing the run and leaving a legible record.
//
// The rest still holds — it is unattended, so assumptions beat questions.
function scheduledPromptSuffix(job) {
  return [
    'This is a scheduled run from the Agent 007 job board' + (job.schedule ? ` (${job.schedule})` : '') + '.',
    'You are in a dedicated git worktree on your own branch, so work directly here.',
    '',
    ...
    '',
    'This is not necessarily a coding task. Do what was asked, and nothing more:',
    'do not open a pull request unless the work itself calls for one. Finish by',
    'writing a short plain-text summary of what you did and what you found, in',
    'this terminal, so it is on the record when someone reads back.',
    '',
    'When that summary is written you are done — stop there. The board notices',
    'the run has finished, closes this agent, releases the worktree, and puts the',
    'card back in To do with its next run time.',
  ];
}
...
// The fields a scheduled card carries back to To do after a run. The agent link
// goes (that agent is being closed), the schedule and the record of the run
// stay, and nextRunAt is measured from NOW rather than from the previous due
// time — so a run that overran its own interval simply schedules the next one
// afterwards instead of coming due again the instant it lands.
//
