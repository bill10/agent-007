...
export function createJob({ title, detail, repoPath, postedBy, postedByName, postedByAgent }) {
  ...
  return {
    job: {
      ...
      // Set when another agent posted the card on a user's behalf (the
      // `agent-007-job` CLI). Kept separate from postedByName rather than
      // folded into it: postedBy* is the human the work belongs to, and the
      // board still needs to show that a machine, not they, typed it.
...
export function buildJobPrompt(job) {
  ...
  parts.push(
    ...
    '',
    'Found something worth doing that is out of scope for this job? Queue it instead',
    'of building it: `agent-007-job "Title" --detail "..."` posts a new card to To do.',
  );
  return parts.join('\n');
}
