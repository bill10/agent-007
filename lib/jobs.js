...
// an unattended dontAsk card comes back prompting and stalls. With no card
// (a manual agent) nothing is added, and each CLI's own default applies.
// Validated through the same allowlist as dispatch, so this is the one place
// that guarantees the mode is a single token and not a smuggled second flag.
export function resumeCommand(agent, mode) {
  const validMode = isValidPermissionMode(mode) ? mode : null;
  if (agent === 'codex') {
    const flag = validMode ? CODEX_MODE_FLAGS[validMode] : '';
    return `codex resume --last${flag ? ` ${flag}` : ''}`;
  }
  return `claude --continue${validMode ? ` --permission-mode ${validMode}` : ''}`;
}
