...
// Board agents run with permissions bypassed. The reason is availability, not
// convenience: `auto` was the earlier default, and auto mode needs the
// classifier, which is not available everywhere the board runs. On a Bedrock or
// Vertex setup, or behind `permissions.disableAutoMode`, `claude` accepts
// `--permission-mode auto` at argv parsing and then fails at runtime. A
// dispatched agent dies seconds after spawn, and because the spawn itself
// succeeded the board records no error: the card sits in In progress reading
// "agent gone" and is never retried.
//
// Do not read this as "the board is unattended now". Two things say otherwise:
//
//  - Claude Code still shows the workspace-trust dialog the first time it runs
//    in any directory, and every board agent gets a brand-new worktree, so it
//    fires on EVERY job. bypassPermissions does not skip it (verified against
//    2.1.250 — see the trust pre-seed entry in TODOS.md). Dispatch still costs
//    one human click per job until that is built.
//  - This mode has availability failures of its own, with the identical silent
//    symptom: managed settings can set `permissions.disableBypassPermissionsMode`,
//    and Claude Code refuses the mode outright when running as root or under
//    sudo. The stuck-card failure class is NOT fixed here, only one trigger for
//    it — see the dispatch-failure entry in TODOS.md.
//
// The cost is real and is recorded in DESIGN.md, not hidden here: an agent in
// this mode runs Bash unprompted as the user, so no in-app rule bounds a
// hostile one, and the board's card-ownership check is a guard against
// misbehaviour rather than a security boundary.
//
// Also worth knowing on a new machine: the docs say the first interactive
// session started in this mode shows a one-time dialog asking the user to
// accept responsibility, saved to user settings thereafter. Until someone has
// answered it by hand, a board agent waits at it. `--dangerously-skip-permissions`
// is defined by those same docs as equivalent to this mode.
