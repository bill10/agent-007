...
// Board agents run with permissions bypassed: they work alone in their own
// worktree on a throwaway branch, and everything they produce lands in a
// reviewable PR, so there is no prompt a human is around to answer anyway.
//
// Not `auto`, which was the earlier default: auto mode needs the classifier,
// and that is not available everywhere the board runs — on a Bedrock or Vertex
// setup, or behind `permissions.disableAutoMode`, `claude` accepts the flag at
// argv parsing and then fails at runtime. A dispatched agent dies seconds after
// spawn, and because the spawn itself succeeded the board records no error: the
// card sits in In progress reading "agent gone" and is never retried.
// bypassPermissions has no such dependency.
//
// One thing to know when moving the board to a new machine: the first
// interactive session started in this mode shows a one-time dialog asking the
// user to accept responsibility, and the acceptance is saved to user settings.
// Until someone has accepted it once by hand, a board agent will sit at that
// dialog instead of working. Same for `--dangerously-skip-permissions`, which
// the docs define as equivalent to this mode.
