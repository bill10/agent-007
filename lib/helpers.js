...
//
// Known limit, accepted: a frame is taken to be the whole pane. Codex has
// not been seen to repaint part of it while a dialog waits (it repaints
// nothing at all then), but a repaint of, say, the status row alone would
// stand in for the pane and drop the bubble until the dialog repaints. The
// full answer is a terminal screen model; see TODOS.md.
...
// `merge`: add closed frames onto the last one instead of replacing it. Set
// by the handler for the two seconds after a resize, when Codex repaints the
// pane and then re-inserts history (its welcome box, a tip) frame by frame,
// so the last frame would otherwise be a box rather than the pane.
export function trackSyncFrames(session, data, now = Date.now(), { merge = false } = {}) {
  ...
  for (;;) {
    ...
    if (open && end !== -1 && (begin === -1 || end < begin)) {
      ...
      if (isRealOutput(text)) {
        const kept = merge && session.lastFrame ? `${session.lastFrame} ${text}` : text;
...
export function detectState(session, { now, stateTimeoutMs, promptPatterns, messagePatterns } = {}) {
  ...
  //
  // The newer source speaks. A frame older than the last real output outside
  // one is a program that stopped painting in frames — a shell tab that ran
  // one such tool and then launched Claude Code — and the window is current
  // again. Codex prints everything, history lines included, inside frames, so
  // for it the frame is always the newer of the two (a tie goes to the frame).
  const frame = typeof session.lastFrame === 'string' && (session.lastFrameAt || 0) >= (session.lastOutputAt || 0)
    ? session.lastFrame : '';
  if (frame) {
    for (const pattern of messages) {
