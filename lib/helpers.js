...
export function createCocktailPool(names = COCKTAILS) {
  ...
  const ordinal = (round) => (round === 2 ? '2nd' : round === 3 ? '3rd' : `${round}th`);
  ...
  return {
    ...
    *candidates(repoPath, maxRounds = 99) {
      const rejected = rejectedByRepo.get(repoPath) ?? new Set();
      for (let round = 1; round <= maxRounds; round++) {
        const prefix = round === 1 ? '' : `${ordinal(round)}-`;
        const pool = shuffled(names).map(n => `${prefix}${n}`);
        yield* pool.filter(n => !rejected.has(n));  // fresh names first
        yield* pool.filter(n => rejected.has(n));   // then retry the hints
      }
    },
