...
export const MESSAGE_PATTERNS = [
  ...
  // Codex's own trust dialog, as 0.156 words it ("› 1. Trust and continue
  // 2. Quit"). Unmatched, it read as WAITING, and an agent message typed into
  // it answered it: the Enter picked option 1 and saved the folder as trusted.
  // (0.155 words it "› 1. Yes, continue  2. No, quit", which "Press Enter to
  // continue" above already catches.)
  /\bTrust\s*and\s*continue\b/i,
  // Codex's hook-trust prompt at startup, when ~/.codex/hooks.json has new or
  // changed hooks ("Hooks need review … Press enter to confirm or esc to go
  // back"), and the hooks browser its option 1 opens ("Press t to trust all").
  // Both read as WAITING, and an agent message's Enter picked "Review hooks".
  /\bHooks\s*need\s*review\b/i,
  /Press\s*enter\s*to\s*confirm\s*or\s*esc/i,
...
export function createCodenamePool(names = CODENAMES) {
  const used = new Set();
  // Names no agent may ever take (Billion's): held for good, whatever else
  // is recycled — an old orphan that happens to carry one included.
...
// A leading ~/ as the home directory, the way a shell would read it.
export function expandHome(path, home = homedir()) {
  const raw = String(path ?? '').trim();
  return raw.startsWith('~/') ? resolve(home, raw.slice(2)) : raw;
}
...
// The environment every pty gets. A server started by launchd, systemd or a
// bare ssh command has no locale, so its terminals ran in POSIX: readline,
// vim and tmux mangled UTF-8, and ssh (which forwards LANG) carried the same
// blindness to the remote host. A UTF-8 LANG fills in only when none is set.
export function ptyEnv(env) {
  const hasLocale = env.LC_ALL || env.LC_CTYPE || env.LANG;
  return { ...env, TERM: 'xterm-256color', ...(hasLocale ? {} : { LANG: 'en_US.UTF-8' }) };
}
...
// An on-by-default switch from the environment: off only for 0/false/off/no.
export function envSwitchOn(value) {
