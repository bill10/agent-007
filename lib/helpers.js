...
export const MESSAGE_PATTERNS = [
  ...
  // codex-cli 0.153 and Claude Code 2.1.)
  ...
  /\bUpdate Model Permissions\b/i,                     // the /permissions picker's title
  /\bYes, (proceed|just this once|continue anyway|grant|and (don't|do not) ask again)\b/i,
  /\bNo, (continue without|and (block|tell))\b/i,      // "No, and tell Codex/Claude what to do differently"
...
export function detectState(session, { now, stateTimeoutMs, promptPatterns, messagePatterns } = {}) {
  ...
  const frame = typeof session.lastFrame === 'string' ? session.lastFrame : '';
  if (frame) {
    for (const pattern of messages) {
