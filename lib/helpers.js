...
export function createCocktailPool(names = COCKTAILS) {
  ...
  const isUsed = (repoPath, name) =>
    reservedFor(repoPath).has(name) || Boolean(takenByRepo.get(repoPath)?.has(name));
  ...
  return {
    ...
    // Accepts full branch names ("bill/vesper") and keeps only the last segment,
    // matching how createWorktree composes `${gitUser}/${cocktail}`.
    //
    ...
    syncFromBranches(repoPath, branchNames) {
      takenByRepo.set(repoPath, new Set(
        branchNames
          .map(b => String(b).trim().split('/').pop())
          .filter(Boolean),
      ));
    },
