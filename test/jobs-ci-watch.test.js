// The CI watch on Review cards: Billion hears once per head commit when CI
...
describe('parsePrCi', () => {
  it('waits while a check runs or none exist, then lists failures from both rollup shapes', () => {
    ...
    expect(p([run('Tests (Ubuntu)', 'SUCCESS'), run('lint', 'SKIPPED')])).toEqual({ failed: [] });
    expect(p([{ ...run('test', 'FAILURE'), workflowName: 'Tests (Windows)' }])).toEqual({ failed: ['Tests (Windows) / test'] });
    expect(p([run('Tests (Ubuntu)', 'FAILURE'), { __typename: 'StatusContext', context: 'ci/legacy', state: 'ERROR' }]))
      .toEqual({ failed: ['Tests (Ubuntu)', 'ci/legacy'] });
...
describe('checkReviewCi', () => {
  it('tells Billion once per head SHA, and a new push re-arms it', async () => {
    ...
    expect(job.ciNotifiedSha).toBe('sha1');
  ...
  it('stays quiet while checks run, and for cards Billion did not post', async () => {
    ...
    expect(theirs.ciNotifiedSha).toBeUndefined();
  });
