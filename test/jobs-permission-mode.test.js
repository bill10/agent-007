...
import {
  ...
  postJobForAgent, editJobForAgent, fireSchedules,
} from '../server/jobs.js';
...
describe('a mode retuned while the agent is spawning', () => {
  ...
  it('dispatches a schedule\'s run in the mode the schedule carries', async () => {
    ...
    const calls = [];
    await dispatchOnce(racingCreateSession(calls, () => {}), noopBroadcast, { killSession: async () => {} });
    expect(calls[0].command).toContain('--permission-mode bypassPermissions');
    expect(job.state).toBe('todo');   // the schedule itself never goes out
  });
