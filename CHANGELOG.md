...
- Jobs dispatched by the board no longer stop at every permission prompt on
  ...
  skips that -- so each job still needs one click to get going. What changes
  is what happens after it: one click at the start instead of a prompt at
  every step. And on a new machine, the first session started with permissions
